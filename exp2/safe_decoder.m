function [ output ] = safe_decoder( input,key )
%SAFE_DECODER 此处显示有关此函数的摘要
%   此处显示详细说明
input = double(input);
...
for n = 0:block_num-1
    ...
    data_mat = reshape(data,[block_size(2),block_size(1)])';
    for t=1:Iterations
        left_part = data_mat(:,1:block_size(2)/2);
        right_part = data_mat(:,block_size(2)/2+1:end);
        left_part_move = left_part;
        for i = 1:size(left_part_move,1)
            left_part_move(i,:)=circshift(left_part_move(i,:)',i-1)';
        end
        left_result1 = conv2(left_part,key);
        left_result1 = left_result1(ceil(size(key,1)/2):end-floor(size(key,1)/2),...
            ceil(size(key,2)/2):end-floor(size(key,2)/2));
        left_result2 = conv2(left_part_move,key_move);
        left_result2 = left_result2(ceil(size(key,1)/2):end-floor(size(key,1)/2),...
            ceil(size(key,2)/2):end-floor(size(key,2)/2));
        left_result = left_result1+left_result2;
        left_result = left_result + right_part;
        left_result = mod(left_result,2);
        right_result = left_part;
