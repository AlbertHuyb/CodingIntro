...
wrong_rate = zeros(10,length(sigma_ns));
Iterates = 3;
...
for n = 1:length(SNR)
    for i = 1:Iterates
        ...
        re = symbol2sequence_for_PSK(re,voltage_num(k),1);
        data = prob(:);
        if voltage_num(k)==4
            %[seq,sym] = viterbi(2,4,[15,17],0,0,data,log2(voltage_num(k)));
            rotate_result = temp_demodulate(out,voltage_num(k),bias_ratio*A,sites);
            [seq] = viterbi_decode_s(rotate_result,[13,15],4,0,2,1);
        elseif voltage_num(k)==8
    ...
    if voltage_num(k)==8
        ...
        wrong_rate(10,n)=wrong_rate(10,n)/Iterates;
    end
    n
end
k
